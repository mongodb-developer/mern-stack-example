...
export default function RecordList() {
	...
	const filterSetter =(value)=>{
		...
		if(flist.includes(value)){
			flist = flist.filter(e=>e != value)
		}
		else{
			flist.push(value)
		}
    ...
    async function filterLevel (){
        console.log("hello ")
        let Records = await fetch(`http://localhost:5050/record?filter=${filterList.join(",")}` )
                    ...
                    <table className="w-full caption-bottom text-sm">
                        <thead className="[&amp;_tr]:border-b">
                            <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                ...
                                <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground [&amp;:has([role=checkbox])]:pr-0">
                                    <div className="px-4 align-middle font-medium text-muted-foreground">
										Level
									<button
                                    ...
                                    {/* <span onClick={Filter}>Fil</span> */}
                                    <div className="filter-container mb-4">
                                    {/* <label for="levelFilter" className="mr-2 font-medium">Filter by Level:</label> */}
                                    
                                      <div
                                    id="dropdown"
                                    className={`absolute mt-1 p-3 border rounded bg-white shadow-md ${toggleDropdown? "" :"hidden"}`}>
        
                                    <div className="flex flex-col space-y-2">
                                     {/* <label className="flex items-center"> */}
                                      {/* <select multiple className="mr-2">
                                     <option value="Junior">Junior</option>
                                     <option value="Senior">Senior</option>
                                     <option value="Intern">Intern</option>
                                    </select> */}
									<label>
                					<input type="checkbox" name="color" value="Junior" id="colorGreen" onChange={()=>{
										filterSetter("Junior")
									}}/>
									 Junior
            						</label>
									<label>
                					<input type="checkbox" name="color" value="Senior" id="colorGreen" onChange={()=>{
										filterSetter("Senior")
									}}/>
									 Senior
            						</label>
									<label>
                					<input type="checkbox" name="color" value="Intern" id="colorGreen" onChange={()=>{
										filterSetter("Intern")
									}}/>
									 Intern
            						</label>
...
>>>>>>> 9b2cca8 (Filter changes modified)
}
